...
function stockTemplate(tab, detail) {
  ...
  return `<section class="stock-view">
    <header class="stock-toolbar"><div class="stock-identity"><h1>${escapeHtml(detail.name)}<span>${escapeHtml(detail.code)}</span></h1><div class="muted mono">${latest ? escapeHtml(latest.time) : "No market history"}${detail.partial ? " · partial data" : ""}</div></div><div class="stock-quote"><span class="stock-price">${formatNumber(latest?.close)}</span><span class="number ${changeClass(change)}">${change == null ? "—" : `${change > 0 ? "+" : ""}${formatNumber(change)}%`}</span></div><div class="periods" aria-label="Chart period">${[["daily","D"],["weekly","W"],["monthly","M"]].map(([period,label]) => `<button class="period-button ${tab.period === period ? "active" : ""}" data-period="${period}">${label}</button>`).join("")}</div><div class="nav-buttons"><button class="ghost-button" data-neighbor="${escapeHtml(activeRows[index - 1]?.code || "")}" ${index <= 0 ? "disabled" : ""}>← Prev</button><button class="ghost-button" data-neighbor="${escapeHtml(activeRows[index + 1]?.code || "")}" ${index < 0 || index >= activeRows.length - 1 ? "disabled" : ""}>Next →</button></div></header>
    ${detail.bars.length ? `<div class="stock-content"><div class="chart-pane"><div class="chart-legend"><span class="ma5">MA5</span><span class="ma10">MA10</span><span class="ma20">MA20</span><span class="ma60">MA60</span></div><div class="chart" id="stock-chart"></div><a class="chart-watermark" href="https://www.tradingview.com/" target="_blank" rel="noopener">Charts by TradingView</a></div>${evidenceTemplate(detail.hits)}</div>` : `<div class="empty-state"><strong>No usable chart history</strong><span>Signal evidence remains available below.</span></div>${evidenceTemplate(detail.hits)}`}
  </section>`;
}
