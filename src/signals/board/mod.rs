...
fn is_limit_up(prev_close: f64, close: f64) -> bool {
    if prev_close <= 0.0 {
        return false;
    }
    (close - prev_close) / prev_close * 100.0 >= LIMIT_UP_PCT
}
...
impl SignalDetector for BrokenBoardSignal {
    ...
    fn detect(&self, bars: &[Candle], _ctx: &StockContext) -> SignalResult {
        ...
        let prev_prev = &bars[n - 3];
        let prev = &bars[n - 2];
        let today = &bars[n - 1];
        let high_hit_limit = prev_prev.close > 0.0
            && (prev.high - prev_prev.close) / prev_prev.close * 100.0 >= LIMIT_UP_PCT;
        let closed_below = prev.close < prev.high * 0.98;
...
impl SignalDetector for StrongFirstNegSignal {
    ...
    fn detect(&self, bars: &[Candle], _ctx: &StockContext) -> SignalResult {
        ...
        let prev = &bars[n - 2];
        let today = &bars[n - 1];
        let prev_was_limit = is_limit_up(prev_prev.close, prev.close);
        let mild_decline =
            today.close < today.open && (today.open - today.close) / today.open * 100.0 < 3.0;
        let above_ma5 = {
